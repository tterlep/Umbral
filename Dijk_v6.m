...
function [distSP,prevS] = Dijk_v6(e_val,grph,Z,zlc,irdir,irmag)
...
for ids = 1:size(source_m,1)
    distS(source_m(ids),source_n(ids)) = 0;
end
prevS =  zeros(size_m,size_n);
prevSmulti = coordvert(source_m,source_n,size_n);
for ips = size(prevSmulti,2)
    prevS(source_m(ips),source_n(ips)) = prevSmulti(ips);
end
